(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) new swiper_core_Swiper(".beforeafter__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        function validateForm() {
            ...
            if (!name) {
                showError("Будь ласка, введіть імʼя");
                return false;
            }
            if (!phoneInput.value) {
                showError("Будь ласка, введіть номер телефону");
                return false;
            }
            if (!phone.startsWith("380") || phone.length !== 12) {
                showError("Номер телефону повинен бути у форматі +380XXXXXXXXX");
                return false;
            }
            return true;
        }
        form.addEventListener("submit", async e => {
            ...
            try {
                const response = await fetch("https://hook.eu1.make.com/no90qbut572z82tckuy4l23j7tyb9hl1", {
                    method: "POST",
                    headers: {
                ...
                if (response.ok) window.location.href = "/thanks.html"; else showError("Помилка відправки. Спробуйте ще раз.");
            } catch (err) {
                console.error(err);
